    ...
    <v-card class="pa-2 pb-10 card" color="white" flat>
      <div class="card-inner-1">
        <v-row style="height: 100%" class="pa-4" no-gutters>
          <v-col cols="10" class="">
            <v-img eager src="../assets/images/dest1.png"></v-img>
          </v-col>
          <v-col cols="2">
            <p class="special-text ml-5 font-6 mt-5">{{ post.date }}</p>
          </v-col>
        </v-row>
      </div>

      <div class="card-inner-2">
        <p class="font-4 weight-600">{{ post.title }}</p>
        <v-row align="center" justify="center" no-gutters>
          <v-chip color="red chip lighten-5" class="pl-3 pr-3" small>
            <router-link :to="getPostPath()" class="font-7 red--text">Read more</router-link>
          </v-chip>
          <v-spacer></v-spacer>
        </v-row>
